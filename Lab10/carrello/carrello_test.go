/*
Scrivere un programma carrello.go che simula gli spostamenti di un carrello lungo un percorso (a celle unitarie) in cui
 sono sono collocati degli oggetti di peso specificato. Il carrello deve avanzare e ripulire il percorso caricando gli 
 oggetti man mano che li incontra, ma senza superare il suo carico massimo. Quando non può caricare un oggetto
  (per motivi di peso), deve andare all'inizio del percorso, scaricare lì (nella cella 0) il suo carico, e ripartire.
...
- una funzione aggiornaStato(c *Carrello, posizione, carico int) bool
  che aggiorna i dati del carrello (numero di cella e carico) e restuisce true, se posizione e carico non sono negativi; 
  altrimenti non fa nessun aggiornamento e restituisce false.

- una funzione main() che legge da file, il cui nome è passato su linea di comando, la descrizione di un percorso 
(a celle) 
con oggetti da rimuovere. Una cella è delimitata da '|'; se è vuota (se c'è uno spazio bianco), in quella posizione 
non c'è nessun oggetto, 
se invece contiene un numero, in quella posizione c'è un oggetto di quel peso. Ad esempio la seguente è 
la descrizione di un percorso:
